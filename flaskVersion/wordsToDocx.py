...
def searchForWordInGroup(dic, word):
    for group in dic:
        for line in group:
...
def searchFromYahoo(voc):
    ...
    for i in range(0, len(lines)):
        line = lines[i]
        if i == 0:
            if len(line.contents) >= 3:
                group.append(voc + ' ' + addParen(line.contents[0].text + line.contents[2].text))
            else:
                group.append(voc + ' ' + '(專有名詞.)' + line.contents[0].text)
        else:
            if len(line.contents) >= 3:
                group.append(addParen(line.contents[0].text + line.contents[2].text))
            else:
                group.append('(專有名詞.)' + line.contents[0].text)
    print(group)
    return group

def makeVocWord(dic, words, savedFileName = None, progress = {'finished' : 0, 'total' : 0, 'finishedAllTheWork' : False}):
    
    progress['finishedAllTheWork'] = False
    
    if savedFileName == None:
        savedFileName = time.strftime("%Y%m%d_%H%M%S", time.localtime()) + '.docx'
        
    if len(savedFileName.split('.')) == 1:
        savedFileName += '.docx'
    elif savedFileName.split('.')[-1] != 'docx' and savedFileName.split('.')[-1] != 'doc':
        savedFileName = savedFileName[:(len(savedFileName) - len(savedFileName.split('.')[-1]) + 1)] + '.docx'
    ...
    for voc in vocList:
        ...
        if voc[-1] == '\n':
            group = searchForWordInGroup(dic, voc[:-1])
        else:
            group = searchForWordInGroup(dic, voc)
            ...
            for string in group:
                ...
                if string[0] == '(':
                    ...
                    if string.find('*ph*') != -1:
                        ...
                        currentRow.cells[1].merge(currentRow.cells[2]);
                        currentRow.cells[1].merge(currentRow.cells[3]);
                        currentRow.cells[1].text = string.split('*')[-1]
    ...
    currentRow.cells[0].merge(currentRow.cells[1])
    currentRow.cells[0].merge(currentRow.cells[2])
    currentRow.cells[0].merge(currentRow.cells[3])
    currentRow.cells[0].text = 'Made by Albert'
    for paragraph in currentRow.cells[0].paragraphs:
    ...
    progress['finishedAllTheWork'] = True
    print('Successfully save ' + savedFileName)
